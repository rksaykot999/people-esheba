          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1.25rem' }}>
              <span style={{
                ...
                {t("education.live_badge") || "Comprehensive School Directory"}
              </span>
            </div>
            ...
            <h1 style={{
              ...
              {t("education.school_title") || "Find the Right School"}<br />
              <span style={{
                background: 'linear-gradient(135deg, #E63946, #ff6b6b)',
                WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
                backgroundClip: 'text',
              }}>
                {t("education.school_highlight") || "For Your Child"}
              </span>{' '}Bangladesh
            </h1>
            <p style={{ color: 'var(--text-muted)', fontSize: '1rem', maxWidth: 500, lineHeight: 1.7, margin: '0 auto 2rem' }}>
              {t("education.school_sub") || "Explore verified information on government schools, private institutions, and madrasas across the country."}
            </p>

            {/* Stats */}
            <div style={{ display: 'flex', justifyContent: 'center', gap: '1.25rem', flexWrap: 'wrap' }}>
              {[
                { label: t("education.stat_schools") || "Schools Listed", value: 1500, suffix: '+', icon: <FiBook size={16} />, color: 'var(--red)' },
                { label: t("education.stat_districts") || "Districts", value: 64, suffix: '', icon: <FiMapPin size={16} />, color: 'var(--cyan)' },
                { label: t("education.stat_verified") || "Verified", value: 95, suffix: '%', icon: <FiShield size={16} />, color: 'var(--green)' },
              ].map(s => (
      ...
      <div className="container" style={{ padding: '2.5rem 1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <input
                ...
                placeholder={t("common.search_placeholder") || "Search schools..."}
                className="form-input"
                style={{ paddingLeft: 42, height: 46, borderRadius: 12, fontSize: '0.9rem' }}
              />
            </div>
            <button type="button" className="btn btn-primary" style={{ height: 46, padding: '0 1.5rem', borderRadius: 12, fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
          <span style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>
            {loading ? t("common.loading") : <><strong style={{ color: 'var(--text)' }}>{filtered.length}</strong> {t("education.found") || "schools found"}</>}
          </span>
          {(typeFilter || search) && (
            <button
        ...
        {loading ? (
          ...
          <div style={{
            ...
            <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text)' }}>{t("education.no_results") || "No schools found"}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '1.75rem' }}>
            ...
            <div>
              <h2 style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--text)' }}>
                {t("education.tips_title") || "Choosing the Right School"}
              </h2>
              <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                {t("education.tips_sub") || "Key factors to consider before making a decision."}
              </p>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '1rem' }}>
            {[
              { Icon: FiMapPin, color: '#E63946', title: t("education.tip_location") || "Location Matters", desc: t("education.tip_location_desc") || "Consider proximity to home and safe transport." },
              { Icon: FiAward, color: '#06B6D4', title: t("education.tip_results") || "Academic Reputation", desc: t("education.tip_results_desc") || "Look at public exam results and teacher quality." },
              { Icon: FiHeart, color: '#10B981', title: t("education.tip_environment") || "Learning Environment", desc: t("education.tip_environment_desc") || "Facilities, play areas, and student support." },
